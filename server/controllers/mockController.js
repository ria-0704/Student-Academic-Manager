...
async function submitAnswer(req, res) {
  ...
  try {
    ...
    // Save attempt
    // answer_type distinguishes typed answers from handwritten/OCR-extracted ones.
    // Defaults to 'typed' — existing callers that don't send it get the default.
    const answerType = req.body.answer_type === 'handwritten' ? 'handwritten' : 'typed';
    const [attemptResult] = await db.query(
      'INSERT INTO mock_attempts (user_id, question_id, student_answer, max_marks, status, answer_type) VALUES (?, ?, ?, ?, ?, ?)',
      [req.user.id, question_id, student_answer.trim(), question.marks, 'submitted', answerType]
    );
    const attemptId = attemptResult.insertId;
